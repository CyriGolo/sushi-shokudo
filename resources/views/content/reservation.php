        ...
        <?php if(isset($_SESSION["reservation"])) { ?>
            <form method="post" action="/confirmation" class="w-full flex flex-col border-3 border-cyan-500 rounded-md">
                ...
                <div class="h-full p-4 flex flex-col gap-4 justify-between">
                    <input type="text" id="titulaire" name="titulaire" placeholder="Titulaire du compte" class="w-full p-1 border-3 border-gray-300 rounded-sm" value="<?= $user ? $user->getName() : ''; ?>" required/>
                    <span class="error"><?php echo error("titulaire"); ?></span>
                    <div class="flex gap-6">
                        <div class="flex flex-col gap-2 w-full">
                          <input type="text" id="card-number" name="card-number" minlength="16" maxlength="16" autocomplete="cc-number" placeholder="Numéro de carte" class="w-full p-1 border-3 border-gray-300 rounded-sm" value="<?= $user ? $user->getNumCarte() : ''; ?>" required/>
                          <span class="error"><?php echo error("card-number"); ?></span>
                        </div>
                        <div class="flex flex-col gap-2 w-full">
                          <input type="text" id="crypto" name="crypto" minlength="3" maxlength="3" autocomplete="cc-csc" placeholder="Cryptograme" class="w-full p-1 border-3 border-gray-300 rounded-sm" value="<?= $user ? $user->getCrypto() : ''; ?>" required/>
                          <span class="error"><?php echo error("crypto"); ?></span>
                        </div>
                    </div>
                    <div class="flex gap-6">
                        <div class="flex flex-col gap-2 w-full">
                          <input type="text" id="facturation" name="facturation" autocomplete="street-address" placeholder="Adresse de facturation" class="w-full p-1 border-3 border-gray-300 rounded-sm" value="<?= $user ? $user->getAddress() : ''; ?>" required/>
                          <span class="error"><?php echo error("facturation"); ?></span>
                        </div>
                        <div class="flex flex-col gap-2 w-full">
                          <input type="tel" id="phone" name="phone" placeholder="Téléphone" autocomplete="tel" class="w-full p-1 border-3 border-gray-300 rounded-sm" value="<?= $user ? $user->getTel() : ''; ?>" required/>
